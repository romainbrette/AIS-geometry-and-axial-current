...
for date, retina, cell, age in zip(dates, retinas, cells, ages): 
    if date < 20190611: # only with P5:
        ...
        if len(row_cell['s1']) > 0:
            ...
            ### Loop on sweeps corresponding to different prepulse potentials
            for rec in sweeps:
                na_rec = int(rec)
                ...
                ### Extract Rs rec, Rs compensation and Vh from the recording database
                ...
                ### Extract Rs from passive properties measurements
                ...
                for sweepNumber in abf_tp.sweepList:
                    ...
                    ### exp fitting
                    idx_end = idx_i_max + int(1.*ms/dt_tp) # fit over the first 1 ms int(t_peak/dt) #
                    i_amp_end = i[idx_end]
                    
                    max_amp = i_amp_end #max(i[idx_i_max:idx_end])
                ...
                # subplot(212)
                # plot(Vc_peaks[idx_peak_ax_current], I_peaks[idx_peak_ax_current], 'ro') 
                ...
                for i in range(sweep_peak, sweep_peak+2): 
                    ...
                    idx_peak = argmin(I_corr_pass[i])
                    # i_peak = I_corr_pass[i][idx_peak]
                    # idx_end_current = where(I_corr_pass[i][idx_peak:] > 0)[0][0] + idx_peak
                
                    # area_idx = arange(idx_end_current)
                    # area_time = t_cut[area_idx]
                    # area_I = I_corr_pass[i][area_idx]   
                    # area = 0
