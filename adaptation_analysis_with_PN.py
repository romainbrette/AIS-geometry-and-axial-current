    ...
    if date >= 20190611: # only with P5:
        ...
        if len(row_cell['s1']) > 0:
            ...
            for rec in sweeps:
                ...
                ### RC time constant estimation from P/N pulses 
                tau_rc = []
                ...
                for i in range(n_rec):
                    ...
                    i_amp_max = i_cut[idx_i_max] 
        
                    # fitting an exponential to the transient decay
                    idx_end = idx_i_max + int(.5*ms/dt) # fit over the first 1 ms idx_i_min #
                    i_amp_end = i_cut[idx_end]
                    
                    max_amp = max(i_cut[idx_i_max:idx_end])
                    
                    ax1.plot(t_cut, i_cut)
                    ax1.set_xlim(idx_start*dt/ms, (idx_start*dt + 3*ms)/ms)
                    ax1.set_xlabel('t (s)')
                    
                    def exp_current(t, tau):
                        return  (i_amp_max - max_amp) * exp(-t/tau) + max_amp
                    
                    idx_i_max = idx_i_max + 1 # starting the fit just after the peak helps, the peak is likely filtered
                    i_range = i_cut[idx_i_max: idx_end] # pA
                    t_range = (t_cut[idx_i_max:idx_end]- t_cut[idx_i_max])  # sec
                    
                    tau_opt = curve_fit(exp_current, t_range, i_range)
                    
                    ax1.plot(t_cut[idx_i_max:idx_end], i_range, 'k-')
                    ax1.plot(t_range + t_cut[idx_i_max], \
                              exp_current(t_range, tau_opt[0]), 'r-')
                    print ('Tau RC:', tau_opt[0])
                ...
                I_corr_pass, I_cut, t_cut = p5_subtraction(date, retina, cell, dt, I, V, f"{na_rec:04d}")
                ...
                i_data = I_corr_pass[idx_peak_ax_current][: idx_peak+20] * 1e-3 #nA #I_high[idx_ax][int(59.*ms/dt):int(60.*ms/dt)]*1e-3 # nA
                ...
                for j in range(n_pts):
                    ...
                    v_last = v_this 
                #print ('Correction factor:', 1-fracCorr) 
