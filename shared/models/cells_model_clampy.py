...
__all__ = [ 'RGCModel', 'AxonalInitiationModel']

class AxonalInitiationModel(BrianExperiment):
    '''
    A spatially extended model with a dendrite and axon, including an initial segment.
    
    Perfect voltage-clamp.
    '''
    def __init__(self, params, resting_vm = -70.*mV, Na_start = 5.*um, Na_end = 35.*um, \
                 density = False, i_inj = 0*amp, gclamp = 10000*usiemens, dt = 0.02*ms):
        ...
        I_VC = gclamp * (Vcommand(t-t_start)-v) * VC_on : amp (point current)
...
class RGCModel(BrianExperiment):
    '''
    A spatially extended model with a dendrite and axon, including an initial segment.
    ...
    '''
    def __init__(self, params, resting_vm = -70.*mV, Na_start = 5.*um, Na_end = 35.*um, \
                 density = False, i_inj = 0*amp, Cm_tot = 50.*pF, gclamp = 1*usiemens, dt = 0.02*ms):
        ...
        print (soma_area)
        Cm = Cm_tot/soma_area
        gL_true = params.gL #10. * (siemens / meter ** 2) # to obtain a realistic input resistance
        #1./(.1*Gohm * soma_area)
        #print gL_true
        ...
        self.neuron.gclamp = 1000*usiemens
